...
def build_agent_instruction(agent_key: str, ctx=None) -> str:
    ...
    return (
        ...
        f"{memory}\n\n"
        "Visible discussion history (public messages only; tool calls, tool results, "
        "and memory updates are intentionally hidden):\n"
        f"{discussion_history}\n\n"
        "Read the visible discussion history and give a short opinion.\n"
        f"Then call the {memory_tool} tool with the full updated Shared Mental Model markdown.\n"
        "Do not include the memory update in your response.\n\n"
...
def build_agent_tool_instruction(agent_key: str, ctx=None) -> str:
    ...
    return (
        ...
        "Task context (use in your answer and memory update):\n"
        f"Goal: {goal}\n"
        f"Candidates: {', '.join(candidates)}\n"
        f"Public information:\n{_as_bullets(public_info)}\n"
        f"Private information:\n{_as_bullets(private_info)}\n\n"
        ...
        "have during a normal discussion turn.\n"
        f"Before your final answer, call the {memory_tool} tool with the full updated "
        "Shared Mental Model markdown reflecting this interaction.\n"
        "Do not include the memory update, metadata, or any special formatting in your response."
    )
