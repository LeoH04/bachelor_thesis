...
def _information_sharing_guidance() -> str:
    ...
    return (
        "Use the limited rounds efficiently. Decide what information to share "
        "or ask about based on your current judgment of the discussion. Apply "
        "the active context transparency policy to determine how much detail, "
        "candidate linkage, reasoning, source context, and uncertainty to "
        "disclose publicly."
    )
...
def _memory_guided_turn_section() -> str:
    ...
    if explicit_smm_memory_enabled():
        return (
            ...
            "Use your current internal memory from the previous update phase to "
            "identify unresolved issues, missing decision-relevant information, "
            ...
            "section indicates a gap that could affect the final candidate "
            "choice, prioritize that gap in a targeted agent-tool question or "
            "in your public contribution, while staying within the active "
            "context transparency policy.\n"
            "Do not update your memory during this speaking turn; use it only "
            "as context for deciding what to ask or say.\n\n"
        )
...
def build_agent_instruction(
    ...
    "Use a tool call when missing, conflicting, or uncertain information could affect the hiring decision.\n"
    ...
    "Tool questions should be specific and decision-relevant.\n"
